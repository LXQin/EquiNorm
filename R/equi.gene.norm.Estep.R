...
function(data.y.simple, data.x, theta.hat, P){
   ...
   dens.Ov		<- compute.dens.D(data.y.simple, data.x, sigma2.hat, tau2.hat, psi2.hat, muO.hat)
   ...
   temp		<- (1-pi.hat)*exp(dens.ND)+pi.hat*lambda.hat*exp(dens.Ov)+pi.hat*(1-lambda.hat)*exp(dens.Un)
   o.hat 		<- pi.hat*lambda.hat*exp(dens.Ov)/temp
   u.hat		<- pi.hat*(1-lambda.hat)*exp(dens.Un)/temp
   z.hat		<- o.hat + u.hat
   ...
   return( list(o.hat=o.hat, u.hat=u.hat, z.hat=z.hat) )
}
